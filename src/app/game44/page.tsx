  ...
  const resolveGame = async () => {
    ...
    try {
      ...
      setTimeout(async () => {
        ...
        try {
          const newGameTx = await program.methods.createGame(new BN(nextMult), name, createdAtSec)
            .accounts({ gameState: newGamePda, signer: wallet, systemProgram: SystemProgram.programId })
            .transaction();
          await sendTransaction(newGameTx, connection);
          showStatus('success', `New game: ${name}`);
          loadAllGames();
        } catch (e: any) {
          showStatus('error', 'Auto-game failed');
        }
      }, 3000);
